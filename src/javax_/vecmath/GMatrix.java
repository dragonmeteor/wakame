...
public class GMatrix implements java.io.Serializable, Cloneable {
	...
	if (nCol != m1.nRow ||  nCol != m1.nCol)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix0"));
	...
	if (m1.nCol != m2.nRow || nRow != m1.nRow || nCol != m2.nCol)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix1"));
     ...
     */
    public final void mul(javax_.vecmath.GVector v1, javax_.vecmath.GVector v2)
    {
	int i, j;

	if (nRow < v1.getSize())
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix2"));

	if (nCol < v2.getSize())
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix3"));
	...
	if (nRow != m1.nRow) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix4"));

	if (nCol != m1.nCol) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix5"));
	...
	if (m2.nRow != m1.nRow) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix6"));

	if (m2.nCol != m1.nCol) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix7"));

	if (nCol != m1.nCol  || nRow != m1.nRow) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix8"));
	...
	if (nRow != m1.nRow)  
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix9"));
 
	if (nCol != m1.nCol) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix28"));
	...
	if (m2.nRow != m1.nRow) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix10"));

	if (m2.nCol != m1.nCol) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix11"));

	if (nRow !=  m1.nRow || nCol != m1.nCol) 
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix12"));
	...
	if (nRow != m1.nRow  || nCol != m1.nCol)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix13"));
     ...
     */  
    public final void getRow(int row, javax_.vecmath.GVector vector)
    {
     ...
     */  
    public final void getColumn(int col, javax_.vecmath.GVector vector)
    {
     ...
     * @param m1  The matrix that will hold the new values
     */  
    public final void get(javax_.vecmath.Matrix4d m1)
    { 
	if (nRow < 4 || nCol < 4) {
	    m1.setZero();
	    if (nCol > 0) {
     ...
     */  
    public final void setRow(int row, javax_.vecmath.GVector vector)
    {
	for(int i = 0; i < nCol; i++) {
     ...
     * @param vector  the source vector
     */  
    public final void setColumn(int col, javax_.vecmath.GVector vector)
    {
	for(int i = 0; i < nRow; i++) {
	...
	if (m1.nRow != m2.nCol || nRow != m1.nCol || nCol != m2.nRow)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix14"));
	...
	if (m1.nCol != m2.nCol || nCol != m2.nRow || nRow != m1.nRow)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix15"));
	...
	if (m1.nRow != m2.nRow || nCol != m2.nCol || nRow != m1.nCol)
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix16"));
    ...
    public final void transpose(GMatrix m1)
    {
	    ...
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix17"));
	...
	for (int i = 0; i < nRow; i++) {
		...
		bits = 31L * bits + javax_.vecmath.VecMathUtil.doubleToLongBits(values[i][j]);
	    }
	}
	...
	if (nCol != V.nCol || nCol != V.nRow) {
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix18"));
	}

	if (nRow != U.nRow || nRow != U.nCol) {
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix25"));
	}

	if (nRow != W.nRow || nCol != W.nCol) {
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix26"));
	}
     ...
     */
    public final int LUD(GMatrix LU, javax_.vecmath.GVector permutation)
    {
	...
	int i, j;
        ...
        if (nRow != nCol) {
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix19"));
        }

        if (nRow != LU.nRow) {
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix27"));
        }
	    ...
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix20"));
        }
	...
	for (i = 0; i < nRow; i++) {
		...
		(javax_.vecmath.VecMathI18N.getString("GMatrix21"));
        }
	...
	if (m1.nRow != m1.nCol) {
	    ...
	    throw new javax_.vecmath.MismatchedSizeException
		(javax_.vecmath.VecMathI18N.getString("GMatrix22"));
	}
	...
	for (j = 0; j < dim; j++) {
	    ...
	    if (imax < 0) {
		throw new RuntimeException(javax_.vecmath.VecMathI18N.getString("GMatrix24"));
	    }
